...
La tienda organiza el catálogo en 4 categorías — Shopify **Collections manuales** (no smart/tags): `ritual-epoch` (Ritual Epoch), `rituales-de-piel` (Rituales de Piel), `tendencia-milito` (Tendencia Milito), `suplementos-y-bienestar` (Suplementos y Bienestar). Los handles los autogenera Shopify a partir del título al crear la collection — no son arbitrarios, están hardcodeados en `COLLECTION_HANDLES` (`lib/shopify.ts`) tal como Shopify los devolvió.
