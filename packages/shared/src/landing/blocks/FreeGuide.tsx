...
import type { LandingFreeGuide } from "@diana-mile/shared/types";
...
export function FreeGuide({ data }: { data: LandingFreeGuide }) {
      ...
      <div className="mx-auto max-w-2xl rounded-2xl border border-dorado/40 bg-crema p-5">
        <div className="flex items-start gap-3">
          ...
          <div className="flex flex-col gap-1">
            ...
            <h3 className="font-display text-xl text-carbon">{data.title}</h3>
            <p className="text-sm text-carbon-suave">{data.description}</p>
            <p className="text-xs text-morado-oscuro font-medium">
              Te la enviamos apenas nos confirmes que recibiste tu pedido — un
              motivo más para avisarnos.
            </p>
          </div>
        </div>
        ...
        {open && (
          <div className="animate-fade-in-up mt-4 flex flex-col gap-4 border-t border-arena pt-4 text-sm text-carbon-suave">
            {data.sections.map((seccion) => (
              <div key={seccion.title}>
                <p className="font-semibold text-carbon">{seccion.title}</p>
                <p>{seccion.body}</p>
              </div>
            ))}
            <p className="text-xs text-ceniza">
              Esto es solo un adelanto — te enviamos la guía completa por
              WhatsApp cuando confirmes que ya recibiste tu pedido.
            </p>
          </div>
        )}
      </div>
