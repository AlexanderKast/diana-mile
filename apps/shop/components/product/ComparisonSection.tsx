...
import type { LandingComparison } from "@diana-mile/shared/types";

export function ComparisonSection({ data }: { data: LandingComparison }) {
    ...
    <section className="bg-crema py-12 px-6">
      ...
      <div className="max-w-md mx-auto rounded-2xl border border-arena bg-blanco overflow-hidden">
        <div className="grid grid-cols-[1fr_auto_auto] items-center gap-3 border-b border-arena bg-crema px-4 py-3">
          <span />
          <span className="text-xs font-semibold text-dorado-oscuro text-center w-16">
            Nosotros
          </span>
          <span className="text-xs font-semibold text-ceniza text-center w-16">
            Otros
          </span>
        </div>
        ...
        {filas.map((fila, index) => (
          ...
          >
            <span className="text-sm text-carbon-suave leading-snug">
              {fila}
            </span>
